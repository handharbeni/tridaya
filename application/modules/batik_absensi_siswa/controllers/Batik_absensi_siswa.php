...
class Batik_absensi_siswa extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT absensi_siswa_batik.*, m_pertemuan_batik.*, m_siswa_batik.nama_lengkap AS nama_siswa, m_unit.nama AS nama_unit FROM absensi_siswa_batik"
      ." LEFT JOIN m_pertemuan_batik ON absensi_siswa_batik.pertemuan_id = m_pertemuan_batik.id" 
      ." LEFT JOIN m_siswa_batik ON absensi_siswa_batik.siswa_id = m_siswa_batik.id" 
      ." LEFT JOIN m_unit ON m_siswa_batik.unit_id = m_unit.id" 
      ." WHERE absensi_siswa_batik.deleted = '0'";
      if($id) { $sql .= " AND absensi_siswa_batik.id = ".$id; }
    $sql .= " ORDER BY absensi_siswa_batik.timestamp DESC";
...
/*      $tableData[] = '<div class="relative mrg-top-10">'
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'absensi_siswa_batik');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'absensi_siswa_batik');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'absensi_siswa_batik', 'id');
