...
class Sekolah_guru extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT m_guru_sekolah.*, m_agama.nama AS nama_agama, m_unit.nama AS nama_unit FROM m_guru_sekolah"
      ." LEFT JOIN m_unit ON m_guru_sekolah.unit_id = m_unit.id" 
      ." LEFT JOIN m_agama ON m_guru_sekolah.agama_id = m_agama.id" 
      ." WHERE m_guru_sekolah.deleted = '0'";
      if($id) { $sql .= " AND m_guru_sekolah.id = ".$id; }
    $sql .= " ORDER BY m_guru_sekolah.timestamp DESC";
  ...
  public function get_guru_by_id()
  {
    ...
    if(isset($params['id'])) {
      ...
      $result = $this->model_adm->select($condition, 'm_guru_sekolah')->row();
      $response = array('status' => 1, 'data' => $result);
    }
    echo json_encode($response);
  }
  ...
  function get_datatable(){
    ...
    $sql = "SELECT m_guru_sekolah.*, m_unit.nama AS nama_unit, m_agama.nama AS nama_agama FROM m_guru_sekolah "
      ." LEFT JOIN m_unit ON m_guru_sekolah.unit_id = m_unit.id "
      ." LEFT JOIN m_agama ON m_guru_sekolah.agama_id = m_agama.id "
      ." WHERE m_guru_sekolah.deleted = 0";
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (m_guru_sekolah.nama_lengkap LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR m_unit.nama LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_sekolah.tempat_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_sekolah.tanggal_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_sekolah.alamat LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_sekolah.no_telp LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_sekolah.no_hp LIKE '%".$requestData['search']['value']."%')";
    }
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'm_guru_sekolah');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'm_guru_sekolah');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_guru_sekolah', 'id');
