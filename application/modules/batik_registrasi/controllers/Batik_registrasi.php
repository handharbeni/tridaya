...
class Batik_registrasi extends MX_Controller {
	...
	public function __construct() 
  {
    ...
    $data['list_unit'] = $this->model_adm->select(array('deleted' => 0), 
      'm_unit')->result();
    $data['list_isian'] = $this->model_adm->select(array('deleted' => 0, 'kategori_unit' => 4), 'm_isian_pertanyaan')->result();//kategori_unit (1=Sekolah, 2=Privat, 3=Bimbel, 4=batik)
  ...
  public function do_add() {
    ...
    if(!empty($arr_data['siswa'])) {
      ...
      $id_siswa = $this->model_adm->insert_id($data_db, 'm_siswa_batik');
      ...
      if($id_siswa) {
        ...
        $result = $this->model_adm->insert_batch($data_db, 'm_orangtua_batik');
        $results['ortu'] = $result;

        //PREPARING DATA JADWAL
        if(!empty($arr_data['jadwal'])){
          $data_db = $arr_data['jadwal'];
          //PREPARING NAMA_MAPEL & NILAI MAPEL
          $data_jadwal = [];
          foreach ($data_db['hari'] as $key => $hari) {
            $data_jadwal[$hari] = $data_db['waktu'][$key];
          }
          unset($data_db['hari']); unset($data_db['waktu']);
          //INSERTING DATA NILAI
          $data_db['siswa_id'] = $id_siswa;
          $data_db['pilihan_jadwal'] = json_encode($data_jadwal);
          $result = $this->model_adm->insert($data_db, 'reg_jadwal_batik');
          $results['jadwal'] = $result;
        }
        ...
        if(!empty($arr_data['isian'])){
          ...
          $result = $this->model_adm->insert($data_db, 'm_isian_batik');
