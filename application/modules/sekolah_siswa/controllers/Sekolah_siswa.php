...
class Sekolah_siswa extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT m_orangtua_sekolah.*, m_agama.nama AS nama_agama FROM m_orangtua_sekolah"
      ." LEFT JOIN m_agama ON m_orangtua_sekolah.agama_id = m_agama.id" 
      ." WHERE m_orangtua_sekolah.deleted = '0'";
      if($id) { $sql .= " AND m_orangtua_sekolah.id = ".$id; }
      if($siswa_id) { $sql .= " AND m_orangtua_sekolah.siswa_id = ".$siswa_id; }
    $sql .= " ORDER BY m_orangtua_sekolah.timestamp DESC";
    return $this->model_adm->rawQuery($sql);
  }
  ...
  public function get_siswa_by_id()
  {
    ...
    if(isset($params['id'])) {
      ...
      $result = $this->model_adm->select($condition, 'm_siswa_sekolah')->row();
  ...
  public function get_isian_by_id()
  {
    ...
    if(!empty($condition)) {
      //First, get pertanyaan keehatan
      $pertanyaan = $this->model_adm->select(array('deleted'=>0, 'kategori_unit'=>1), 'm_isian_pertanyaan')->result();//kategori_unit (1=Sekolah, 2=Privat, 3=Bimbel, 4=batik)
    ...
    foreach ($query->result_array() as $row) {
      ...
      $tableData[] = '<div class="relative mrg-top-10">'
                      .'<div class="btn-group">'
                        ...
                        .'<a href="javascript:void(0);" class="btn btn-default text-info edit-notif" data-id="'.$row["id"].'" onclick="showDetailKesehatan(event);" title="Detail Kesehatan"> <i class="fa fa-heart"></i> </a>'
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'm_siswa_sekolah');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'm_siswa_sekolah');
  ...
  public function do_edit_ortu() {
    ...
    if(!empty($params['id'])) {
      ...
      unset($data_db['id']);
      unset($data_db['idSiswa']);
      $result = $this->model_adm->update($condition, $data_db, 'm_orangtua_sekolah');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_siswa_sekolah', 'id');
  ...
  public function do_delete_ortu() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_orangtua_sekolah', 'id');
