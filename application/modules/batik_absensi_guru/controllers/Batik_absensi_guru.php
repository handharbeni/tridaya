...
class Batik_absensi_guru extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT absensi_guru_batik.*, m_pertemuan_batik.*, m_guru_batik.nama_lengkap AS nama_guru, m_unit.nama AS nama_unit FROM absensi_guru_batik"
      ." LEFT JOIN m_pertemuan_batik ON absensi_guru_batik.pertemuan_id = m_pertemuan_batik.id" 
      ." LEFT JOIN m_guru_batik ON absensi_guru_batik.guru_id = m_guru_batik.id" 
      ." LEFT JOIN m_unit ON m_guru_batik.unit_id = m_unit.id" 
      ." WHERE absensi_guru_batik.deleted = '0'";
      if($id) { $sql .= " AND absensi_guru_batik.id = ".$id; }
    $sql .= " ORDER BY absensi_guru_batik.timestamp DESC";
  ...
  function get_datatable(){
    ...
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (absensi_guru_batik.tempat LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR absensi_guru_batik.waktu LIKE '%".$requestData['search']['value']."%'";
...
/*      $tableData[] = '<div class="relative mrg-top-10">'
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'absensi_guru_batik');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'absensi_guru_batik');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'absensi_guru_batik', 'id');
