...
class Batik_jadwal extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT m_jadwal_batik.*, m_pertemuan_batik.*, m_siswa_batik.nama_lengkap AS nama_siswa, m_guru_batik.nama_lengkap AS nama_guru, m_unit.nama AS nama_unit FROM m_jadwal_batik"
      ." LEFT JOIN m_unit ON m_jadwal_batik.unit_id = m_unit.id" 
      ." LEFT JOIN m_pertemuan_batik ON m_jadwal_batik.pertemuan_id = m_pertemuan_batik.id" 
      ." LEFT JOIN m_siswa_batik ON m_pertemuan_batik.siswa_id = m_siswa_batik.id" 
      ." LEFT JOIN m_guru_batik ON m_pertemuan_batik.guru_id = m_guru_batik.id" 
      ." WHERE m_jadwal_batik.deleted = '0'";
      if($id) { $sql .= " AND m_jadwal_batik.id = ".$id; }
    $sql .= " ORDER BY m_jadwal_batik.timestamp DESC";
  ...
  function get_datatable(){
    ...
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (m_jadwal_batik.tempat LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR m_jadwal_batik.start_date LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_unit.nama LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_batik.nama_lengkap LIKE '%".$requestData['search']['value']."%'";
...
/*      $tableData[] = '<div class="relative mrg-top-10">'
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'm_jadwal_batik');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'm_jadwal_batik');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_jadwal_batik', 'id');
