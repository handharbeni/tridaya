...
<body class="header-info">
      ...
      <div class="page-container">
        ...
        <div class="main-content">
            ...
            <div class="page-title">
              <h4>Siswa Batik
                <div class="pull-right">
                  <a href="<?php echo base_url()?>registrasi/batik/" class="btn btn-rounded btn-success edit-notif" title="Tambah data"><i class="ti-plus pdd-right-5"></i> Tambah</a>
          ...
          <div class="modal fade" id="modalForm">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <form action="<?php echo base_url()?>batik/siswa/do_add" method="POST" id="formAdd">      
          ...
          <div class="modal fade" id="modalFormOrtu">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <form action="<?php echo base_url()?>batik/siswa/do_edit_ortu" method="POST" id="formAddOrtu">      
                  <div class="modal-header">
          ...
          <!-- Modal Detail JADWAL START-->
          <div class="modal fade" id="modalJadwal">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 id="modalJadwalHeader">Detail Jadwal Pilihan</h4>
                </div>
                <div class="modal-body">
                 <div class="row">
                  <div class="col-md-12">
  ...
  <script>
    ...
    var initDataTable = $('#tableData').DataTable({
      ...
      "order": [[1, 'ASC']],
      "ajax":{
            url :"<?php echo base_url()?>batik/siswa/get_datatable",
    ...
    function getDetail(id, callback) {
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>batik/siswa/get_siswa_by_id',
          data: { id: id },
          type: 'POST',
          dataType: 'json',
          beforeSend: function() { },
          success: function(response, status) {
            if(response.status) {
              ...
              $('#nilai_rapor').val(data.nilai_rapor);
      ...
      onChange: function(val) {
        ...
        if(!val.length){ 
          ...
          selectizeKecamatan.load(function(callback) {
            ...
            xhr && xhr.abort();
            xhr = $.post("<?php echo base_url()?>batik/siswa/get_kecamatan_by_kota", {id: val}, function(response, status) {
                  if(status == 'success') {
      ...
      onChange: function(val) {
        ...
        if(!val.length){ selectizeKelurahan.disable(); }
          ...
          selectizeKelurahan.load(function(callback) {
            ...
            xhr && xhr.abort();
            xhr = $.post("<?php echo base_url()?>batik/siswa/get_kelurahan_by_kecamatan", {id: val}, function(response, status) {
    ...
    function showModalForm(e) {
      ...
      if(!id) {
        $('#formAdd').attr('action', "<?php echo base_url()?>batik/siswa/do_add");
        $('#modalFormHeader').text('Tambah Siswa Batik');
        $('#modalForm').modal('show');
      }
      //jika klik tombol edit data:
      else {
        $('#formAdd').attr('action', "<?php echo base_url()?>batik/siswa/do_edit");
        $('#modalFormHeader').text('Ubah Siswa Batik');
        getDetail(id);
      }
    }
    //SHOW MODAL FORM
    function showModalFormOrtu(e) {
      ...
      //jika klik tombol tambah data:
      if(!id) {
        $('#formAddOrtu').attr('action', "<?php echo base_url()?>batik/siswa/do_add_ortu");
        $('#modalFormOrtuHeader').text('Tambah Orang Tua Siswa Batik');
        $('#modalFormOrtu').modal('show');
      }
      //jika klik tombol edit data:
      else {
        $('#formAddOrtu').attr('action', "<?php echo base_url()?>batik/siswa/do_edit_ortu");
        $('#modalFormOrtuHeader').text('Ubah Orang Tua Siswa Batik');
        getDetailOrtu(id);
      }
    }
    ...
    function showDetailOrtu(e) {
      ...
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>batik/siswa/get_ortu_by_id',
          data: { siswa_id : id },
          type: 'POST',
          dataType: 'json',
    ...
    // SHOW MODAL DETAIL JADWAL
    function showDetailJadwal(e) {
      e.preventDefault();
      var id = $(e.currentTarget).data('id') || null;
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>batik/siswa/get_jadwal_by_id',
          data: { siswa_id : id },
          type: 'POST',
          dataType: 'json',
          beforeSend: () => { $.LoadingOverlay("show"); },
          success: function(response, status) {
            if(response.status) {
              ...
              let jadwal = (data) ? JSON.parse(data.pilihan_jadwal) : [];
              let no = 1;
              var html = '';
              html += '<table class="table table-striped">';
              html += '<thead><tr> <th>#</th> <th>Hari</th> <th>Waktu</th> </tr></thead>';
              html += '<tbody>';
              $.map(jadwal, function(row, i) {
                html += '<tr>'
                    +'<td>'+no+'</td>'
                    +'<td>'+i+'</td>'
                    +'<td>'+row+'</td>';
    ...
    function showDetailIsian(e) {
      ...
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>batik/siswa/get_isian_by_id',
          data: { siswa_id : id },
          type: 'POST',
          dataType: 'json',
          beforeSend: () => { $.LoadingOverlay("show"); },
    ...
    function doMultiDelete(arrData, targetTable='') {
      ...
      let url = "<?php echo base_url()?>batik/siswa/do_delete";
      if(targetTable == 'ortu') {
        url = "<?php echo base_url()?>batik/siswa/do_delete_ortu";
      }
      if(ids[0] != null) {
