...
<body class="header-info">
          ...
          <div class="modal fade" id="modalForm">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <form action="<?php echo base_url()?>master/guru/do_add" method="POST" id="formAdd">      
  ...
  <script>
    ...
    var initDataTable = $('#tableData').DataTable({
      ...
      "order": [[1, 'ASC']],
      "ajax":{
            url :"<?php echo base_url()?>master/guru/get_datatable",
    ...
    function getDetail(id, callback) {
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>master/guru/get_guru_by_id',
          data: { id: id },
          type: 'POST',
          dataType: 'json',
          beforeSend: function() { },
      ...
      onChange: function(val) {
        ...
        if(!val.length){ 
          ...
          selectizeKecamatan.load(function(callback) {
            ...
            xhr && xhr.abort();
            xhr = $.post("<?php echo base_url()?>master/guru/get_kecamatan_by_kota", {id: val}, function(response, status) {
                  if(status == 'success') {
      ...
      onChange: function(val) {
        ...
        if(!val.length){ selectizeKelurahan.disable(); }
          ...
          selectizeKelurahan.load(function(callback) {
            ...
            xhr && xhr.abort();
            xhr = $.post("<?php echo base_url()?>master/guru/get_kelurahan_by_kecamatan", {id: val}, function(response, status) {
    ...
    function showModalForm(e) {
      ...
      if(!id) {
        $('#formAdd').attr('action', "<?php echo base_url()?>master/guru/do_add");
        $('#modalFormHeader').text('Tambah Guru');
        $('#modalForm').modal('show');
      }
      //jika klik tombol edit data:
      else {
        $('#formAdd').attr('action', "<?php echo base_url()?>master/guru/do_edit");
    ...
    function doMultiDelete(arrData, targetTable='') {
      ...
      let url = "<?php echo base_url()?>master/guru/do_delete";
      if(targetTable == 'ortu') {
        url = "<?php echo base_url()?>master/guru/do_delete_ortu";
      }
      if(ids[0] != null) {
