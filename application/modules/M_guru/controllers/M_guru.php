...
class M_guru extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT m_guru.*, m_agama.nama AS nama_agama, m_unit.nama AS nama_unit FROM m_guru"
      ." LEFT JOIN m_unit ON m_guru.unit_id = m_unit.id" 
      ." LEFT JOIN m_agama ON m_guru.agama_id = m_agama.id" 
      ." WHERE m_guru.deleted = '0'";
      if($id) { $sql .= " AND m_guru.id = ".$id; }
    $sql .= " ORDER BY m_guru.timestamp DESC";
  ...
  public function get_guru_by_id()
  {
    ...
    if(isset($params['id'])) {
      ...
      $result = $this->model_adm->select($condition, 'm_guru')->row();
      $response = array('status' => 1, 'data' => $result);
    }
    echo json_encode($response);
  }
  ...
  function get_datatable(){
    ...
    $sql = "SELECT m_guru.*, m_unit.nama AS nama_unit, m_agama.nama AS nama_agama FROM m_guru "
      ." LEFT JOIN m_unit ON m_guru.unit_id = m_unit.id "
      ." LEFT JOIN m_agama ON m_guru.agama_id = m_agama.id "
      ." WHERE m_guru.deleted = 0";
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (m_guru.nama_lengkap LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR m_unit.nama LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru.tempat_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru.tanggal_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru.alamat LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru.no_telp LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru.no_hp LIKE '%".$requestData['search']['value']."%')";
    }
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'm_guru');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'm_guru');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_guru', 'id');
