...
class Batik_guru extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT m_guru_batik.*, m_agama.nama AS nama_agama, m_unit.nama AS nama_unit FROM m_guru_batik"
      ." LEFT JOIN m_unit ON m_guru_batik.unit_id = m_unit.id" 
      ." LEFT JOIN m_agama ON m_guru_batik.agama_id = m_agama.id" 
      ." WHERE m_guru_batik.deleted = '0'";
      if($id) { $sql .= " AND m_guru_batik.id = ".$id; }
    $sql .= " ORDER BY m_guru_batik.timestamp DESC";
  ...
  public function get_guru_by_id()
  {
    ...
    if(isset($params['id'])) {
      ...
      $result = $this->model_adm->select($condition, 'm_guru_batik')->row();
      $response = array('status' => 1, 'data' => $result);
    }
    echo json_encode($response);
  }
  ...
  function get_datatable(){
    ...
    $sql = "SELECT m_guru_batik.*, m_unit.nama AS nama_unit, m_agama.nama AS nama_agama FROM m_guru_batik "
      ." LEFT JOIN m_unit ON m_guru_batik.unit_id = m_unit.id "
      ." LEFT JOIN m_agama ON m_guru_batik.agama_id = m_agama.id "
      ." WHERE m_guru_batik.deleted = 0";
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (m_guru_batik.nama_lengkap LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR m_unit.nama LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_batik.tempat_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_batik.tanggal_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_batik.alamat LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_batik.no_telp LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_batik.no_hp LIKE '%".$requestData['search']['value']."%')";
    }
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'm_guru_batik');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'm_guru_batik');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_guru_batik', 'id');
