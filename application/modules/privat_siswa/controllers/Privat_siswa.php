...
class Privat_siswa extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT m_siswa_private.* FROM m_siswa_private"
      ." WHERE m_siswa_private.deleted = '0'";
      if($id) { $sql .= " AND m_siswa_private.id = ".$id; }
    $sql .= " ORDER BY m_siswa_private.timestamp DESC";
    return $this->model_adm->rawQuery($sql);
  }
  public function get_data_list_ortu($id=null, $siswa_id=null)
  {
    $sql = "SELECT m_orangtua_private.* FROM m_orangtua_private"
      ." WHERE m_orangtua_private.deleted = '0'";
      if($id) { $sql .= " AND m_orangtua_private.id = ".$id; }
      if($siswa_id) { $sql .= " AND m_orangtua_private.siswa_id = ".$siswa_id; }
    $sql .= " ORDER BY m_orangtua_private.timestamp DESC";
    return $this->model_adm->rawQuery($sql);
  }
  ...
  public function get_siswa_by_id()
  {
    ...
    if(isset($params['id'])) {
      ...
      $result = $this->model_adm->select($condition, 'm_siswa_private')->row();
  ...
  public function get_ortu_by_id()
  {
    ...
    $condition = []; $join = [];
    if(isset($params['id'])) {
      $condition['m_orangtua_private.id'] = $params['id'];
    }
    if(isset($params['siswa_id'])) {
      $condition['m_orangtua_private.siswa_id'] = $params['siswa_id'];
    }
    if(!empty($condition)) {
      $select = 'm_orangtua_private.*';
      // $join[] = ['m_agama', 'm_agama.id = m_orangtua_private.agama_id', 'left'];
      $condition['m_orangtua_private.deleted'] = 0;

      $result = $this->model_adm->select_join($condition, 'm_orangtua_private', $select, $join)->result();
      $response = array('status' => 1, 'data' => $result);
    }
    // print('<pre>'.print_r($response, true).'</pre>');
    echo json_encode($response);
  }
  public function get_nilai_by_id()
  {
    $params = $this->input->post();
    $response = $this->response;
    $condition = [];
    ...
    if(!empty($condition)) {
      //First, get nilai
      $data_nilai = $this->model_adm->select($condition, 'reg_nilai_private')->row();
  ...
  public function get_isian_by_id()
  {
    ...
    if(!empty($condition)) {
      ...
      $pertanyaan = $this->model_adm->select(array('deleted'=>0, 'kategori_unit'=>2), 'm_isian_pertanyaan')->result();//kategori_unit (1=Sekolah, 2=Privat, 3=Bimbel, 4=batik)
      $jawaban = $this->model_adm->select($condition, 'm_isian_private')->row();
  ...
  function get_datatable(){
    ...
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (m_siswa_private.nama_lengkap LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR m_siswa_private.tempat_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_siswa_private.tanggal_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_siswa_private.no_telp LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_siswa_private.no_hp LIKE '%".$requestData['search']['value']."%')";
    }
    ...
    foreach ($query->result_array() as $row) {
      ...
      $tableData[] = '<div class="relative mrg-top-10">'
                      .'<div class="btn-group">'
                        ...
                        .'<a href="javascript:void(0);" class="btn btn-default text-info edit-notif" data-id="'.$row["id"].'" onclick="showDetailNilai(event);" title="Detail Nilai"> <i class="fa fa-graduation-cap"></i> </a>'
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'm_siswa_private');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'm_siswa_private');
  ...
  public function do_edit_ortu() {
    ...
    if(!empty($params['id'])) {
      ...
      unset($data_db['id']);
      unset($data_db['idSiswa']);
      $result = $this->model_adm->update($condition, $data_db, 'm_orangtua_private');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_siswa_private', 'id');
  ...
  public function do_delete_ortu() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_orangtua_private', 'id');
