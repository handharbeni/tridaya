...
<body class="header-info">
      ...
      <div class="page-container">
        ...
        <div class="main-content">
            ...
            <div class="page-title">
              <h4>Siswa Privat
                <div class="pull-right">
                  <a href="<?php echo base_url()?>registrasi/private" class="btn btn-rounded btn-success edit-notif" title="Tambah data"><i class="ti-plus pdd-right-5"></i> Tambah</a>
          ...
          <div class="modal fade" id="modalForm">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <form action="<?php echo base_url()?>privat/siswa/do_add" method="POST" id="formAdd">      
          ...
          <div class="modal fade" id="modalFormOrtu">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <form action="<?php echo base_url()?>privat/siswa/do_edit_ortu" method="POST" id="formAddOrtu">      
                  <div class="modal-header">
          ...
          <!-- Modal Detail NILAI START-->
          <div class="modal fade" id="modalNilai">
            <div class="modal-dialog modal-lg" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 id="modalNilaiHeader">Detail Nilai</h4>
                </div>
                <div class="modal-body">
                 <div class="row">
                  <div class="col-md-12">
  ...
  <script>
    ...
    var initDataTable = $('#tableData').DataTable({
      ...
      "order": [[1, 'ASC']],
      "ajax":{
            url :"<?php echo base_url()?>privat/siswa/get_datatable",
    ...
    function getDetail(id, callback) {
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>privat/siswa/get_siswa_by_id',
          data: { id: id },
          type: 'POST',
          dataType: 'json',
          beforeSend: function() { },
   ...
   /* $selectizeProvinsi = $('#provinsi').selectize({
    ...
    function showModalForm(e) {
      ...
      if(!id) {
        $('#formAdd').attr('action', "<?php echo base_url()?>privat/siswa/do_add");
        $('#modalFormHeader').text('Tambah Siswa Privat');
        $('#modalForm').modal('show');
      }
      //jika klik tombol edit data:
      else {
        $('#formAdd').attr('action', "<?php echo base_url()?>privat/siswa/do_edit");
        $('#modalFormHeader').text('Ubah Siswa Privat');
        getDetail(id);
      }
    }
    //SHOW MODAL FORM
    function showModalFormOrtu(e) {
      ...
      //jika klik tombol tambah data:
      if(!id) {
        $('#formAddOrtu').attr('action', "<?php echo base_url()?>privat/siswa/do_add_ortu");
        $('#modalFormOrtuHeader').text('Tambah Orang Tua Siswa Privat');
        $('#modalFormOrtu').modal('show');
      }
      //jika klik tombol edit data:
      else {
        $('#formAddOrtu').attr('action', "<?php echo base_url()?>privat/siswa/do_edit_ortu");
        $('#modalFormOrtuHeader').text('Ubah Orang Tua Siswa Privat');
        getDetailOrtu(id);
      }
    }
    ...
    function showDetailOrtu(e) {
      ...
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>privat/siswa/get_ortu_by_id',
          data: { siswa_id : id },
          type: 'POST',
          dataType: 'json',
    ...
    // SHOW MODAL DETAIL NILAI
    function showDetailNilai(e) {
      e.preventDefault();
      var id = $(e.currentTarget).data('id') || null;
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>privat/siswa/get_nilai_by_id',
          data: { siswa_id : id },
          type: 'POST',
          dataType: 'json',
          beforeSend: () => { $.LoadingOverlay("show"); },
          success: function(response, status) {
            if(response.status) {
              ...
              let nilai_mapel = (data) ? JSON.parse(data.nilai_mapel) : [];
              let no = 1;
              var html = '';
              html += '<table class="table table-striped table-sm">';
              html += '<thead><tr> <th>#</th> <th>Nama Mapel</th> <th>Nilai</th> </tr></thead>';
              html += '<tbody>';
              $.map(nilai_mapel, function(row, i) {
                html += '<tr>'
                    +'<td>'+no+'</td>'
                    +'<td>'+i+'</td>'
                    +'<td>'+row+'</td>';
              ...
              html += '<table class="table table-striped table-sm">'
                +'<tbody>'
                  +'<tr>'
                    +'<th>Mapel yang disuka</th>'
                    +'<td>'+data.mapel_suka+'</td>'
                  +'</tr>'
                  +'<tr>'
                    +'<th>Mapel yang dibenci</th>'
                    +'<td>'+data.mapel_benci+'</td>'
                  +'</tr>'
                +'</tbody></table>'
    ...
    function showDetailIsian(e) {
      ...
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>privat/siswa/get_isian_by_id',
          data: { siswa_id : id },
          type: 'POST',
          dataType: 'json',
          beforeSend: () => { $.LoadingOverlay("show"); },
    ...
    function doMultiDelete(arrData, targetTable='') {
      ...
      let url = "<?php echo base_url()?>privat/siswa/do_delete";
      if(targetTable == 'ortu') {
        url = "<?php echo base_url()?>privat/siswa/do_delete_ortu";
      }
      if(ids[0] != null) {
