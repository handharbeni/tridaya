...
class Privat_jadwal extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT m_jadwal_private.*, m_pertemuan_private.*, m_siswa_private.nama_lengkap AS nama_siswa, m_guru_private.nama_lengkap AS nama_guru, m_unit.nama AS nama_unit FROM m_jadwal_private"
      ." LEFT JOIN m_unit ON m_jadwal_private.unit_id = m_unit.id" 
      ." LEFT JOIN m_pertemuan_private ON m_jadwal_private.pertemuan_id = m_pertemuan_private.id" 
      ." LEFT JOIN m_siswa_private ON m_pertemuan_private.siswa_id = m_siswa_private.id" 
      ." LEFT JOIN m_guru_private ON m_pertemuan_private.guru_id = m_guru_private.id" 
      ." WHERE m_jadwal_private.deleted = '0'";
      if($id) { $sql .= " AND m_jadwal_private.id = ".$id; }
    $sql .= " ORDER BY m_jadwal_private.timestamp DESC";
  ...
  function get_datatable(){
    ...
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (m_jadwal_private.tempat LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR m_jadwal_private.start_date LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_unit.nama LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_guru_private.nama_lengkap LIKE '%".$requestData['search']['value']."%'";
    ...
    foreach ($query->result_array() as $row) {
      ...
      $tableData[] = '<div class="list-info mrg-top-10">'
                        ...
                        .'</div>';
      $tableData[] = '<div class="list-info mrg-top-10">'
                        .'<p>'.date("d/m/Y H:i", strtotime($row["start_date"]))
...
/*      $tableData[] = '<div class="relative mrg-top-10">'
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'm_jadwal_private');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'm_jadwal_private');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_jadwal_private', 'id');
