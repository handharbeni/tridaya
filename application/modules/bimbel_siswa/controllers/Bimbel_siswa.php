...
class Bimbel_siswa extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT m_siswa_bimbel.*, m_agama.nama AS nama_agama FROM m_siswa_bimbel"
      ." LEFT JOIN m_agama ON m_siswa_bimbel.agama_id = m_agama.id" 
      ." WHERE m_siswa_bimbel.deleted = '0'";
      if($id) { $sql .= " AND m_siswa_bimbel.id = ".$id; }
    $sql .= " ORDER BY m_siswa_bimbel.timestamp DESC";
    return $this->model_adm->rawQuery($sql);
  }
  public function get_data_list_ortu($id=null, $siswa_id=null)
  {
    $sql = "SELECT m_orangtua_bimbel.* FROM m_orangtua_bimbel"
      ." WHERE m_orangtua_bimbel.deleted = '0'";
      if($id) { $sql .= " AND m_orangtua_bimbel.id = ".$id; }
      if($siswa_id) { $sql .= " AND m_orangtua_bimbel.siswa_id = ".$siswa_id; }
    $sql .= " ORDER BY m_orangtua_bimbel.timestamp DESC";
    return $this->model_adm->rawQuery($sql);
  }
  ...
  public function get_siswa_by_id()
  {
    ...
    if(isset($params['id'])) {
      ...
      $result = $this->model_adm->select($condition, 'm_siswa_bimbel')->row();
      $response = array('status' => 1, 'data' => $result);
    }
    echo json_encode($response);
  }
  public function get_ortu_by_id()
  {
    ...
    $condition = []; $join = [];
    if(isset($params['id'])) {
      $condition['m_orangtua_bimbel.id'] = $params['id'];
    }
    if(isset($params['siswa_id'])) {
      $condition['m_orangtua_bimbel.siswa_id'] = $params['siswa_id'];
    }
    if(!empty($condition)) {
      $select = 'm_orangtua_bimbel.*';
      // $join[] = ['m_agama', 'm_agama.id = m_orangtua_bimbel.agama_id', 'left'];
      $condition['m_orangtua_bimbel.deleted'] = 0;

      $result = $this->model_adm->select_join($condition, 'm_orangtua_bimbel', $select, $join)->result();
      $response = array('status' => 1, 'data' => $result);
    }
  ...
  public function get_isian_by_id()
  {
    ...
    if(!empty($condition)) {
      ...
      $pertanyaan = $this->model_adm->select(array('deleted'=>0, 'kategori_unit'=>3), 'm_isian_pertanyaan')->result();//kategori_unit (1=Sekolah, 2=Privat, 3=Bimbel, 4=batik)
      $jawaban = $this->model_adm->select($condition, 'm_isian_bimbel')->row();
  ...
  function get_datatable(){
    ...
    $sql = "SELECT m_siswa_bimbel.* FROM m_siswa_bimbel "
      ." WHERE m_siswa_bimbel.deleted = 0"; 
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (m_siswa_bimbel.nama_lengkap LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR m_siswa_bimbel.tempat_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_siswa_bimbel.tanggal_lahir LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_siswa_bimbel.no_telp LIKE '%".$requestData['search']['value']."%'";
      $sql.=" OR m_siswa_bimbel.no_hp LIKE '%".$requestData['search']['value']."%')";
    }
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'm_siswa_bimbel');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'm_siswa_bimbel');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'm_siswa_bimbel', 'id');
