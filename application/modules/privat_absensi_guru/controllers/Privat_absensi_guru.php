...
class Privat_absensi_guru extends MX_Controller {
  //Global variable
  var $data;
  private $response;
  ...
  {
    $sql = "SELECT absensi_guru_private.*, m_pertemuan_private.*, m_guru_private.nama_lengkap AS nama_guru, m_unit.nama AS nama_unit FROM absensi_guru_private"
      ." LEFT JOIN m_pertemuan_private ON absensi_guru_private.pertemuan_id = m_pertemuan_private.id" 
      ." LEFT JOIN m_guru_private ON absensi_guru_private.guru_id = m_guru_private.id" 
      ." LEFT JOIN m_unit ON m_guru_private.unit_id = m_unit.id" 
      ." WHERE absensi_guru_private.deleted = '0'";
      if($id) { $sql .= " AND absensi_guru_private.id = ".$id; }
    $sql .= " ORDER BY absensi_guru_private.timestamp DESC";
  ...
  function get_datatable(){
    ...
    if(!empty($requestData['search']['value'])) {
      $sql.=" AND (absensi_guru_private.tempat LIKE '%".$requestData['search']['value']."%'"; 
      $sql.=" OR absensi_guru_private.waktu LIKE '%".$requestData['search']['value']."%'";
...
/*      $tableData[] = '<div class="relative mrg-top-10">'
  ...
  public function do_add() {
    ...
    if(!empty($params)) {
      ...
      $result = $this->model_adm->insert($data_db, 'absensi_guru_private');
  ...
  public function do_edit() {
    ...
    if(!empty($params['id'])) {
      ...
      $result = $this->model_adm->update($condition, $data_db, 'absensi_guru_private');
  ...
  public function do_delete() {
    ...
    if(!empty($params['ids'])) {
      ...
      $result = $this->model_adm->update_batch($data_db, 'absensi_guru_private', 'id');
