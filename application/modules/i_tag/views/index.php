...
<body class="header-info">
          ...
          <div class="modal fade" id="modalForm">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <form action="<?php echo base_url()?>informasi/tag/do_add" method="POST" id="formAdd">      
                  <div class="modal-header">
                    <h4 id="modalFormHeader">Tambah Tag</h4>
                  </div>
                   ...
                   <div class="row">
                     <div class="col-sm-12">
                        <div class="form-group">
                         ...
                         <input type="text" name="nama" id="nama" class="form-control" placeholder="Nama Tag" required="">
                         <input type="hidden" id="id" class="form-control" placeholder="ID Tag">
                       </div>
                     </div>
                     <!-- <div class="col-sm-12">
                        <div class="form-group">
  ...
  <script>
    ...
    loadTabelTag(jsonList);
    
    function loadTabelTag(json){
      //clear table
    ...
    function getDetail(id, callback) {
      if(id) {
        $.ajax({
          url: '<?php echo base_url();?>informasi/tag/get_tag_by_id',
          data: { id: id },
          type: 'POST',
          dataType: 'json',
          beforeSend: function() { },
    ...
    function showModalForm(e) {
      ...
      if(!id) {
        $('#formAdd').attr('action', "<?php echo base_url()?>informasi/tag/do_add");
        $('#formAdd :input').val('');
        $('#modalFormHeader').text('Tambah Tag');
        $('#modalForm').modal('show');
      }
      //jika klik tombol edit data:
      else {
        $('#formAdd').attr('action', "<?php echo base_url()?>informasi/tag/do_edit");
        $('#modalFormHeader').text('Ubah Tag');
        getDetail(id);
      }
    }
      ...
      if(ids[0] != null) {
        ...
        function(isConfirm){
          if(isConfirm) {
            //Hapus banyak data
            $.ajax({
              url: "<?php echo base_url()?>informasi/tag/do_delete",
              data: { ids: ids },
              type: 'POST',
              dataType: 'json',
              beforeSend: function() { 
